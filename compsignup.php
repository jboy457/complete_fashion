<?php 
        ...
        include 'complete_func.php';
        $formerror = [];

       if (array_key_exists('signup', $_POST)) {
            
            if (empty($_POST['firstname'])) {
                  $formerror['firstname'] = '<p>Please enter your firstname<p>';
            }else{
                $fn = $_POST['firstname'];
            }
            if (empty($_POST['lastname'])) {
              $formerror['lastname'] = '<p>Please enter your lastname</p>';
            }else{
                $ln = $_POST['lastname'];
            }
            if (empty($_POST['username'])) {
               $formerror['username'] = '<p>Please enter a username</p>';
            }else{
                $us = $_POST['username'];
            }
            if (empty($_POST['email'])) {
                $formerror['email'] = '<p>Please enter your email</p>';
            }else{
                $em = $_POST['email'];
            }
            if (empty($_POST['password'])) {
                $formerror['password'] = '<p>Please enter your password</p>';
            }else{
                $ps = $_POST['password'];
            }
...
<form action="compsignup.php" method="POST">
    <div id="login-box">
  <div class="left">
    <a class="already_account" href="already_account.php">Already have an account?</a>
    <h1>Sign up</h1>
   <p><input type="text" name="firstname" placeholder="Firstname" />
        <?php displayErrors('firstname', $formerror); ?></p>
    <p><input type="text" name="lastname" placeholder="Lastname" />
        <?php 
              displayErrors('lastname', $formerror);
            ?></p>
   <p><input type="text" name="username" placeholder="Username" />
        <?php 
              displayErrors('username', $formerror);
            ?></p>
    <p><input type="text" name="email" placeholder="E-mail" />
        <?php 
            displayErrors('email', $formerror);
            ?></p>
    <p><input type="password" name="password" placeholder="Password" />
        <?php 
              displayErrors('password', $formerror);
            ?></p>
    
    <input type="submit" name="signup" value="Sign me up" />
  </div>
  
  <div style="background-image: url(bg.jpg);" class="right">
    <span  class="loginwith">Sign in with<br />social network</span>
    
    <button class="social-signin facebook" ><a  href="https://www.facebook.com" style="text-decoration: none;">Sign up with facebook</a></button>
    <button class="social-signin twitter"><a href="https://www.twitter.com" style="text-decoration: none;">Sign up with Twitter</a></button>
    <button class="social-signin google"><a href="https://accounts.google.com/servicelogin" style="text-decoration: none;">Sign up with Google+</a></button>
  </div>
  <div class="or">OR</div>
    </div>
</form>    
